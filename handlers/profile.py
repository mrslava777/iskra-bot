...
async def on_edit(call: CallbackQuery, state: FSMContext) -> None:
    ...
    # FIX: отдельные хендлеры для del_voice и cancel_voice
    if field == "del_voice":
    ...
    if field == "verify":
        user = await db.get_user(call.from_user.id)
        # FIX: проверяем verified через get, т.к. aiosqlite.Row — это не dict
        verified = user.get("verified") if hasattr(user, "keys") else False
        if verified:
