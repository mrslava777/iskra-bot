...
log = logging.getLogger("iskra.profile")
...
async def _send_profile(message: Message, user) -> None:
    ...
    has_voice_flag = _has_voice(user)
    try:
        await message.answer_photo(
            photo=user["photo_id"], caption=caption, reply_markup=profile_kb(has_daily, has_voice_flag)
        )
    except Exception:
        await message.answer(caption, reply_markup=profile_kb(has_daily, has_voice_flag))
...
# ---------- Отдельные хендлеры для голосового (ДО общего ed: хендлера!) ----------

@router.callback_query(F.data == "ed:del_voice")
async def on_del_voice(call: CallbackQuery, state: FSMContext) -> None:
    """Удаление голосового приветствия."""
...
@router.callback_query(F.data == "ed:cancel_voice")
async def on_cancel_voice(call: CallbackQuery, state: FSMContext) -> None:
    """Отмена записи голосового."""
    await state.clear()
...
async def on_edit(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if field == "voice":
        ...
        if has_voice_flag:
            await call.message.answer(
                ...
                "У тебя уже есть голосовое в анкете.\n"
                "Отправь новое голосовое (до 1 минуты), чтобы заменить.\n\n"
                "Или нажми /cancel для отмены.",
                reply_markup=voice_cancel_kb(),
            )
        else:
            await call.message.answer(
                "🎙 <b>Голосовое приветствие</b>\n\n"
                "Отправь голосовое сообщение (до 1 минуты).\n"
                "Расскажи о себе, своих увлечениях или просто поздоровайся!\n\n"
                "Или нажми /cancel для отмены.",
                reply_markup=voice_cancel_kb(),
            )
        await call.answer()
    ...
    if field == "del_daily":
        ...
        if user:
            ...
            caption = "👤 <b>Твоя анкета</b>\n\n" + profile_caption(user)
            try:
                await call.message.edit_caption(caption=caption, reply_markup=profile_kb(False, has_voice_flag))
            except Exception:
                try:
                    await call.message.edit_text(text=caption, reply_markup=profile_kb(False, has_voice_flag))
                except Exception:
                    pass
        return
...
# ---------- Обработка голосового при редактировании ----------

@router.message(Edit.voice, F.voice)
async def edit_voice(message: Message, state: FSMContext) -> None:
    voice = message.voice
    if voice.duration > 60:
        await message.answer("Слишком длинно! Максимум 1 минута. Попробуй ещё раз.", reply_markup=voice_cancel_kb())
        return
    try:
        ...
        log.error(f"Error saving voice: {e}")
        await message.answer("❌ Ошибка при сохранении голосового. Попробуй ещё раз.", reply_markup=voice_cancel_kb())


@router.message(Edit.voice, Command("cancel"))
async def edit_voice_cancel(message: Message, state: FSMContext) -> None:
    await state.clear()
...
async def edit_voice_invalid(message: Message) -> None:
    await message.answer(
        ...
        "Отправь голосовое или нажми /cancel для отмены.",
        reply_markup=voice_cancel_kb(),
    )
