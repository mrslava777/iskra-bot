...
async def on_edit(call: CallbackQuery, state: FSMContext) -> None:
    ...
    # Исключаем специальные колбэки, которые обрабатываются отдельными хендлерами
    if field in ("del_voice", "cancel_voice"):
        return  # пропускаем, пусть ловят отдельные хендлеры
    ...
    if field == "voice":
        ...
        # Проверяем, есть ли уже голосовое
    ...
    if field == "verify":
        user = await db.get_user(call.from_user.id)
        # FIX: aiosqlite.Row — доступ по ключу как dict
        try:
...
# NEW: отдельные хендлеры для голосового (ВАЖНО: без F.data.startswith!)
