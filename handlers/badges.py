...
router = Router()
log = logging.getLogger("iskra.badges")
...
async def cmd_badges(message: Message) -> None:
    ...
    try:
        ...
        log.error("Failed to load user %d for badges: %s", message.from_user.id, e)
    ...
    try:
        ...
        log.error("Failed to check/award badges for %d: %s", message.from_user.id, e)
        badges = []
...
async def cb_collection(call: CallbackQuery) -> None:
    ...
    try:
        ...
        try:
            ...
            log.debug("Failed to edit collection text: %s", e)
    except Exception as e:
        log.error("Failed to update collection for %d: %s", call.from_user.id, e)
    await call.answer()
...
async def cb_progress(call: CallbackQuery) -> None:
    ...
    try:
        ...
        try:
            ...
            log.debug("Failed to edit progress text: %s", e)
    except Exception as e:
        log.error("Failed to show progress for %d: %s", call.from_user.id, e)
    await call.answer()
...
async def cb_back_to_menu(call: CallbackQuery) -> None:
    ...
    try:
        ...
        log.debug("Failed to delete message: %s", e)
    await call.answer()
