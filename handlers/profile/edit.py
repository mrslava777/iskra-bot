...
@router.callback_query(F.data.startswith(f"{CallbackPrefix.EDIT.value}:"))
async def on_edit_field(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if field == EditField.INTERESTS.value:
        ...
        await call.message.edit_text(f"{EMOJI.INTERESTS} Выбери интересы:", reply_markup=interests_kb(sel, CallbackPrefix.EDIT_INTEREST.value))
    ...
    if field in prompts:
        ...
        await call.answer("Пока не реализовано")
    await call.answer()
...
async def edit_interests(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if payload == "done":
        ...
        user = await user_repo.get_user(call.from_user.id)
        caption = await format_profile_async(user, show_compat=False, show_badges=True)
        await call.message.edit_text(f"✅ Интересы обновлены!\n\n{caption}")
        return
