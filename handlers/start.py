...
from keyboards import MAIN_MENU, extra_photos_kb, gender_kb, interests_kb, seeking_kb, voice_reg_kb
...
async def reg_photo(message: Message, state: FSMContext) -> None:
    ...
    # NEW: голосовое приветствие
    await state.set_state(Reg.voice)
    await message.answer(
        "🎙 <b>Голосовое приветствие</b>

"
        "Запиши короткое голосовое (до 1 минуты) — расскажи о себе, "
        "своих увлечениях или просто поздоровайся.
"
        "Это делает анкету живой и уникальной!

"
        "Или пропусти, если пока не хочешь:",
        reply_markup=voice_reg_kb(),
    )


# NEW: обработка голосового при регистрации
@router.callback_query(Reg.voice, F.data == "regvoice:skip")
async def reg_skip_voice(call: CallbackQuery, state: FSMContext) -> None:
...
@router.callback_query(Reg.voice, F.data == "regvoice:record")
async def reg_voice_prompt(call: CallbackQuery, state: FSMContext) -> None:
    await call.message.edit_text(
        "🎙 Отправь голосовое сообщение (до 1 минуты):
"
        "Нажми и удерживай кнопку микрофона 🎙"
    )
    await call.answer()


@router.message(Reg.voice, F.voice)
async def reg_voice_received(message: Message, state: FSMContext) -> None:
    voice = message.voice
    ...
    await message.answer("✅ Голосовое сохранено!")
    await state.update_data(extra_count=0)
    await state.set_state(Reg.extra_photos)
    await message.answer(
        "📸 Хочешь добавить ещё фото? (до 4 дополнительных)
...
@router.message(Reg.voice)
async def reg_voice_invalid(message: Message) -> None:
    await message.answer(
...
async def _finish_registration(message: Message, user_id: int, state: FSMContext) -> None:
    ...
    voice_note = "\n🎙 Голосовое приветствие есть" if user and user["voice_id"] else ""
    await message.answer_photo(
        photo=user["photo_id"],
        caption=f"✨ Готово! Вот твоя анкета:\n\n{profile_caption(user)}{photo_note}{voice_note}",
    )
    await message.answer(
...
async def cmd_help(message: Message) -> None:
    await message.answer(
        ...
        "• 🎙 Голосовое приветствие — слышь голос до первого сообщения\n"
