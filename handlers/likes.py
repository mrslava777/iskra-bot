...
from services.matching import gender_emoji, is_night_mode, profile_caption
...
async def show_incoming(message: Message) -> None:
    ...
    night = is_night_mode()
    caption = profile_caption(first, viewer=user, show_compat=True, night_mode=night)
    if night:
        caption = "\U0001F319 <i>Ночной режим — имена скрыты до рассвета</i>
...
async def _show_next_incoming(message: Message, viewer_id: int) -> None:
    ...
    night = is_night_mode()
    caption = profile_caption(nxt, viewer=user, show_compat=True, night_mode=night)
    if night:
        caption = "\U0001F319 <i>Ночной режим — имена скрыты до рассвета</i>
...
async def show_matches(message: Message) -> None:
    ...
    for r in rows:
        ...
        if night:
            ...
            display_name = night_nickname(r["tg_id"])
            contact = f"{display_name} — {contact}
<i>Имя откроется с рассветом \u2600\uFE0F</i>"
        else:
            display_name = r["name"]

        caption = profile_caption(r, viewer=viewer, show_compat=True, night_mode=night)
        caption += f"

📨 Контакт: {contact}"
        if night:
            caption = "\U0001F319 <i>Ночной мэтч</i>
