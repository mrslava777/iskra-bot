...
async def cmd_support(message: Message, state: FSMContext) -> None:
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🧧 Жалоба на пользователя", callback_data="sup:report")],
            [InlineKeyboardButton(text="🗣 Нарушение моих прав", callback_data="sup:rights")],
            [InlineKeyboardButton(text="❓ Другое", callback_data="sup:other")],
            [InlineKeyboardButton(text="↩️ Назад", callback_data="sup:back")],
        ]
    )
    await message.answer(SUPPORT_INTRO, reply_markup=kb)
